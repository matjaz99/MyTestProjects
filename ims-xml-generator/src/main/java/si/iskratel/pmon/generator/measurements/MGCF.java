...
public class MGCF extends IMSNodeSimulator {
	...
	private void simulateCallsCsOrig() {
		...
		CC_AttCallCsOrig = Util.getNextValue(CC_AttCallCsOrig, 1000, 5000, 100);
		CC_FailCallCsOrig = Util.getNextValue(CC_FailCallCsOrig, 0, 1000, 100);
		CC_SuccCallCsOrig = CC_AttCallCsOrig - CC_FailCallCsOrig;
		CC_AnsCallCsOrig = Util.getNextValue(CC_AnsCallCsOrig, CC_FailCallCsOrig, CC_SuccCallCsOrig, 10);
	...
	private void simulateRelCall() {
		...
		CC_AttCallImOrig = Util.getNextValue(CC_AttCallImOrig, 1000, 5000, 100);
		CC_FailCallImOrig = Util.getNextValue(CC_FailCallImOrig, 0, 1000, 100);
		CC_SuccCallImOrig = CC_AttCallCsOrig - CC_FailCallImOrig;
		CC_AnsCallImOrig = Util.getNextValue(CC_AnsCallImOrig, CC_FailCallImOrig, CC_SuccCallImOrig, 10);
	...
	private void simulateCallsImOrig() {
		...
		CC_RelCallCsInit = Util.getNextValue(CC_RelCallCsInit, 0, 2000, 200);
		CC_RelCallsCsInit = Util.getNextValue(CC_RelCallsCsInit, 0, 1000, 100);
		CC_RelCallsMgcfInit = Util.getNextValue(CC_RelCallsMgcfInit, 0, 500, 10);
		CC_RelCallsMgwInit = Util.getNextValue(CC_RelCallsMgwInit, 0, 500, 10);
	...
	private void simulateNbrAnsCall() {
		...
		CC_NbrAnsCallMax = Util.getNextValue(CC_NbrAnsCallMax, 0, 2000, 200);
		CC_NbrAnsCallMean = Util.getNextValue(CC_NbrAnsCallMean, 0, 1000, 100);
